...
from utils import Feature_Dataset_GenePrdt, get_detailed_metrics, create_stratified_grouped_cv_splits
...
def dataPrep_MLecdna(ecDNA_df, cancer_type):
    ...
    return Feature_Dataset_GenePrdt(file_paths,targets), group_ids


def dataPrep_DeepPT(cancer_type,ecDNA_df,gene_file_info,true_gene_expression):
...
def nested_cv_with_regularization_grouped(dataset, input_dim, group_ids, n_outer_folds=5, n_inner_folds=5, 
    ...
    all_labels = [dataset[i][1][1] for i in range(len(dataset))]  
    ...
    for outer_fold, (train_val_idx, test_idx) in enumerate(outer_splits):
        ...
        # Get labels for the train_val samples
        train_val_labels = [all_labels[i] for i in train_val_idx]
        
        # Create INNER CV splits with groups constraints
        inner_splits = create_stratified_grouped_cv_splits(train_val_idx, train_val_labels, train_val_group_ids, n_splits=n_inner_folds)
...
if __name__ == '__main__':
    ...
    true_expression_df = pd.read_csv(args.base_input_path 
                + f"/orginal_data_deepPt/true_gene_expression_mudra/TCGA_{cancer_type}.DeepPT_normalized_reformatted_true_expression_per_sample_{exp_model}.txt",
                delimiter='\t')
    # Gene info file
    genes_info_file = args.base_input_path + f"/orginal_data_deepPt/true_gene_expression_mudra/gene_correlation_auc/cancer_type_gene_list.txt"
    ...
    if args.data_type == 'DeepPt':
        ...
        ecDNA_dataset, groupids = dataPrep_DeepPT(cancer_type,ecDNA_df,genes_info_file,true_expression_df)
    else:
    ...
    results = nested_cv_with_regularization_grouped(ecDNA_dataset,1024,groupids, epochs=32)
